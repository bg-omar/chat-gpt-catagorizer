// ==UserScript==
// @name         Capture ChatGPT Bearer Token
  ...
  window.fetch = async function (...args) {
    ...
    if (config?.headers) {
      ...
      if (auth?.startsWith('Bearer ')) {
        ...
        console.log('%c🐻 Bearer Token:', 'color: lime;', token);
...
(function () {
  ...
  async function getBearerTokenOrPrompt() {
    ...
    return new Promise((resolve) => {
      ...
      label.textContent = "Enter Bearer Token:";
      label.style.display = 'block';
      label.style.marginBottom = '6px';

      const input = document.createElement('input');
      input.type = 'text';
      input.placeholder = 'Paste full Bearer token...';
      input.style.cssText = 'width: 400px; padding: 5px;';

      const button = document.createElement('button');
      button.textContent = 'Submit';
      button.style.cssText = 'margin-top: 6px; padding: 4px 12px;';

      button.onclick = () => {
        const raw = input.value.trim();
        if (!raw.startsWith('Bearer ')) {
          alert('Must start with "Bearer "');
          return;
        }
  ...
  function waitForSidebarReady(callback) {
    const historyAside = document.querySelector('#history > aside');
    if (historyAside) return callback();
    setTimeout(() => waitForSidebarReady(callback), 500);
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (storedConversations.length > 0) {
      updateUIWithStorageConversations(storedConversations); // ✅ unchanged
    }
    initializeButtons();
    setStates();
    waitForSidebarReady(async () => {
      initializeMutationObserver(); // ✅ you'll update selector inside
      initializeButtonClickListeners(); // ✅ same
      const token = await getBearerTokenOrPrompt();
      console.log('✅ Using token:', token);
      fetchConversations().then(repeater); // ✅ fixed call
    });
  ...
  function getHistorySection() {
    return (
        ...
        document.querySelector('aside[aria-labelledby]')
    );
  }
  ...
  function waitForSidebar(callback) {
    const el = document.querySelector('#history > aside');
    if (el) return callback(el);
    setTimeout(() => waitForSidebar(callback), 500);
  }
  ...
  function sortLists() {
    ...
    for (const category in categories) {
      if (Array.isArray(categories[category])) {
        ...
        console.warn(`Skipping sort: categories[${category}] is not an array`, categories[category]);
      }
    }
  ...
  function createCategoryContainer(category, items, color) {
    ...
    newOlContainer.className = '#history > aside';
    ...
    li.innerHTML = `
      <div draggable="true" class="no-draggable group rounded-lg active:opacity-90 bg-[var(--item-background-color)] h-9 text-sm relative" style="--item-background-color: #171717;">
        <a class="flex items-center gap-2 p-2" data-discover="true" href="/c/${conversation.id}">
          <div class="conversation-entry" dir="auto" title="${conversation.title}">
            ${conversation.title}
          </div>
        </a>
