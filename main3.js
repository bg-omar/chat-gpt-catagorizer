// === Fixed ChatGPT Sidebar Enhancer Script === //

let isScriptEnabled = sessionStorage.getItem('isScriptEnabled') === null
    ? true
    : JSON.parse(sessionStorage.getItem('isScriptEnabled'));
let isScrollEnabled = sessionStorage.getItem('isScrollEnabled') === null
    ? true
    : JSON.parse(sessionStorage.getItem('isScrollEnabled'));
...
function patchFetch() {
    ...
    window.fetch = async function (...args) {
        ...
        if (typeof resource === 'string' && resource.includes('/backend-api/conversations')) {
            ...
            const existing = JSON.parse(sessionStorage.getItem('conversations')) || [];
            const newItems = data.items.map(i => ({ ...i, update_time: i.update_time ? new Date(i.update_time) : null }));
            const merged = mergeAndCleanConversations(existing, newItems);
            sessionStorage.setItem('conversations', JSON.stringify(merged));
...
function getStates() {
    ...
    isScrollEnabled = safe('isScrollEnabled', true);
...
function triggerScrollAndEvent() {
    const scrollContainer = document.querySelector('#history');
...
function checkAndReplaceText() {
    const elements = document.querySelectorAll('#history span[dir="auto"]');
    if (!elements.length) return;

    const colors = ['#f0f', '#FF7E00', '#64edd3', '#0f0', '#3cc', '#ff0', '#f00'];
    let colorIndex = 0;
    ...
    elements.forEach(el => {
        ...
        if (match && match[1]) {
            ...
            el.innerHTML = text.replace(
                /\[(.*?)\]/g,
                (_, w) => `<span class="highlighted" style="color:${color};border:1px dotted ${color}">${w}</span>`
            );
            el.closest('li')?.setAttribute('data-category', word);
        }
    });
...
function repeater() {
    const interval = setInterval(async () => {
        if (isPaused) return;
        getStates();
        if (isScrollEnabled) await triggerScrollAndEvent();
        if (isScriptEnabled) await checkAndReplaceText();
        if (isSortListsEnabled) await sortLists();
        setStates();
        if (apiOffset >= dataTotal) clearInterval(interval);
    }, 5000);
}
...
function mergeAndCleanConversations(existing, incoming) {
    const seen = new Set();
    return [...existing, ...incoming].filter(c => {
...
function waitForContainerToLoad(callback, maxRetries = 50, retryCount = 0) {
    const container = document.querySelector('#history > aside');
    if (container) {
