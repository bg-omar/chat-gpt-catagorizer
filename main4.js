...
let isScriptEnabled = sessionStorage.getItem('isScriptEnabled') === null
    ? true
    : JSON.parse(sessionStorage.getItem('isScriptEnabled'));
let isNavScrollEnabled = sessionStorage.getItem('isNavScrollEnabled') === null
    ? true
    : JSON.parse(sessionStorage.getItem('isNavScrollEnabled'));

let isSortListsEnabled = sessionStorage.getItem('isSortListsEnabled') === null
    ? false
    : JSON.parse(sessionStorage.getItem('isSortListsEnabled'));
...
function waitForListContainer(callback, retries = 20) {
    const container = document.querySelector('ol > li')?.closest('div.flex-col') || document.querySelector('#history > aside');
    if (container) {
        callback(container);
    } else if (retries > 0) {
        setTimeout(() => waitForListContainer(callback, retries - 1), 200);
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    patchFetch(); // must run immediately

    waitForReadyDOM(() => {
        if (!isScriptEnabled) {
            // Clean up any UI
            document.querySelectorAll('div[style*="position: fixed"][style*="right: 40px"]').forEach(el => el.remove());
            log('🧹 Script disabled: UI elements removed.');
            return;
        }

        log('✅ DOM Ready — Initializing script...');
        initializeScript(); // will run patchFetch again safely
    });
});

function waitForReadyDOM(callback, retries = 50, delay = 200) {
    const container1 = document.querySelector('#history > aside > a');
    const container2 = document.querySelector('.flex.flex-col.gap-2.text-token-text-primary.text-sm');
    const container3 = document.querySelector('ol > li')?.closest('div');

    if (container1 || container2 || container3) {
        log('⏳ DOM container detected.');
        callback();
    } else if (retries > 0) {
        log(`⏳ Waiting for containers... (${retries})`);
        setTimeout(() => waitForReadyDOM(callback, retries - 1, delay), delay);
    } else {
        console.warn('⚠️ DOM readiness timeout. Initialization skipped.');
    }
}
...
function getStates() {
    ...
    isScriptEnabled = safe('isScriptEnabled', true);
    isNavScrollEnabled = safe('isNavScrollEnabled', true);
    isSortListsEnabled = safe('isSortListsEnabled', false);
...
function sortLists() {
    ...
    olListsToCategorize.forEach((ol) => {
        ...
        listItems.forEach((item) => {
            ...
            const fallbackDate = date2 || new Date(0);
            const date = date2 || (dateStr ? new Date(dateStr) : fallbackDate);
    ...
    // ❗️Ensure a fallback ol if none exists (useful for orphan recovery)
    let olElement = olListsToCategorize[0];
    if (!olElement) {
        olElement = document.createElement('ol');
        listContainer.appendChild(olElement); // 🔧 FIXED: actually append fallback ol
    }
    ...
    // 🧠 Sort all items within their categories by most recent date
    for (const cat in categories) {
        categories[cat].sort((a, b) => b.date - a.date);
    }

    // 🧹 Deduplicate uncategorized items by ID
    const seenIds = new Set();
    const uniqueUncategorizedItems = uncategorizedItems.filter(({ item }) => {
    ...
    // Sort uncategorized by most recent date
    uniqueUncategorizedItems.sort((a, b) => b.date - a.date);

    const fragment = document.createDocumentFragment();

    // 📂 Add uncategorized section first
    if (uniqueUncategorizedItems.length > 0) {
        const uncategorizedOl = createCategoryContainer(
            'Uncategorized',
            uniqueUncategorizedItems.map(obj => obj.item)
        );
        fragment.appendChild(uncategorizedOl);
    }

    // 📁 Collect single-item categories separately
    const sortedCategories = collectSingleItems(categories, singleItems, fragment);

    // 📅 Sort categories by their most recent update
    sortedCategories.sort((a, b) => {
        const dateA = a.mostRecentDate || new Date(0);
        const dateB = b.mostRecentDate || new Date(0);
        return dateB - dateA;
    });

    // 🧱 Add sorted category sections to fragment
    sortedCategories.forEach(({ category, items }) => {
        const container = createCategoryContainer(category, items, wordColors[category]);
        fragment.appendChild(container);
    });

    // 🔄 Remove all but the first original <ol>
    listContainer.querySelectorAll('ol').forEach((ol, idx) => {
        if (idx > 0) ol.parentElement?.remove();
    });

    // 📥 Add new structured sections
    listContainer.appendChild(fragment);

    // ♻️ Reconnect interactivity
